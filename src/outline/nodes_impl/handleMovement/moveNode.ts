...
import { handleContainerSourceMove } from './sourceContainerMove_old';
...
export async function moveNode (
    ...
    if (moverType === 'container') {
        return handleContainerSourceMove('move', this, provider, newParent);
    }
    ...
    if (destinationContainer.getUri().toString() === moverParentUri.toString()) {
        ...
        try {
            const swapResult = await handleContainerSwap(
                'move', this, 
                provider, provider as any as UriBasedView<OutlineNode>,
