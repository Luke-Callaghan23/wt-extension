...
import { ChapterNode, ContainerNode, OutlineNode, SnipNode } from './panels/treeViews/outline/outlineNodes';
...
export class FileAccessManager {
    ...
    static lastAccess: string | undefined;

    // container uri -> uri of last accessed fragment of that container
    private static fileAccesses: { [ index: string ]: string };
    ...
    static async documentOpened (document: vscode.TextDocument, view?: OutlineView) {
        ...
        const openedUri = document.uri.fsPath;
        let uri: string | undefined = openedUri;
		let node: OutlineNode | undefined | null = outlineView._getTreeElementByUri(uri);
		while (node && uri) {
        ...
        FileAccessManager.fileAccesses[containerUri] = openedUri;

        // Also update the latest file access
        FileAccessManager.lastAccess = openedUri;
    }

    // Gets the last accessed document inside of a container
    // If none of a container's 
    static containerLastAccessedDocument (container: OutlineNode): string {
        
        const containerUri = container.getUri();
        
        // First, check if there is a log for this container 
        const lastAccess: string | undefined = FileAccessManager.fileAccesses[containerUri];
        ...
        FileAccessManager.fileAccesses[containerUri] = fragmentUri;
        return fragmentUri;
    }
    ...
    static async initialize () {
        ...
        FileAccessManager.lastAccess = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.document.uri.fsPath : undefined;
