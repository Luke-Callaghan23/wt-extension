 ...
 *--------------------------------------------------------*/
/*
 * extension.ts (and activateTTSDebug.ts) forms the "plugin" that plugs into VS Code and contains the code that
...
import { activateTTSDebug, workspaceFileAccessor } from './activateTTSDebug';
...
export function activateDebug(context: vscode.ExtensionContext) {
	...
	switch (runMode) {
		case 'server':
			...
			activateTTSDebug(context, new TTSDebugAdapterServerDescriptorFactory());
			break;

		case 'namedPipeServer':
			// run the debug adapter as a server inside the extension and communicate via a named pipe (Windows) or UNIX domain socket (non-Windows)
			activateTTSDebug(context, new TTSDebugAdapterNamedPipeServerDescriptorFactory());
			break;

		case 'external': default:
			// run the debug adapter as a separate process
			activateTTSDebug(context, new DebugAdapterExecutableFactory());
			break;

		case 'inline':
			// run the debug adapter inside the extension and directly talk to it
			activateTTSDebug(context);
			break;
	}
}
...
class TTSDebugAdapterServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new TTSDebugSession(workspaceFileAccessor);
...
class TTSDebugAdapterNamedPipeServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
